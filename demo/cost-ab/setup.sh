...
DEMO_NAME=cost-ab
: "${FURYPIPE_DEMO_VARIANT:=[1m]}"   # this demo wants the large-context variant;
...
LOG_ON="$HOME/.furypipe/ab-on.jsonl"
LOG_OFF="$HOME/.furypipe/ab-off.jsonl"
DUMP_DIR="/tmp/furypipe-ab-png"   # FuryPipe arm dumps every rendered PNG here for debug/inspection (wiped each run)
...
FURYPIPE_LOG="$LOG_ON"  FURYPIPE_PORT="$PORT_ON"  FURYPIPE_MODELS="$MODELS" FURYPIPE_DUMP_DIR="$DUMP_DIR" nohup node dist/node.js >/tmp/ab-on.log  2>&1 & disown
FURYPIPE_LOG="$LOG_OFF" FURYPIPE_PORT="$PORT_OFF" FURYPIPE_MODELS="$MODELS" FURYPIPE_DISABLE=1            nohup node dist/node.js >/tmp/ab-off.log 2>&1 & disown
sleep 2
...
Then, in TWO separate terminals:
  ...
  bash demo/cost-ab/b.sh        # RIGHT = FuryPipe   (interactive)
