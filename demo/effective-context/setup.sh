...
: "${FURYPIPE_DEMO_VARIANT:=[1m]}"   # this demo floods the context, so it wants the
                                  # large-context variant; scope keys ignore [tags]
. demo/models.sh

PORT_ON="$DEMO_PORT_ON"    # FuryPipe      -> b.sh (right)
PORT_OFF="$DEMO_PORT_OFF"  # passthrough -> a.sh (left, plain but logged)
LOG_ON="$HOME/.furypipe/ec-on.jsonl"
LOG_OFF="$HOME/.furypipe/ec-off.jsonl"
DUMP_DIR="/tmp/furypipe-ec-png"   # FuryPipe arm dumps every rendered PNG here for debug/inspection (wiped each run)
...
#   FURYPIPE_DEMO_MODEL=... bash setup.sh  -> same, via env
...
# entry and FuryPipe would quietly stop compressing.
...
: >"$LOG_ON"; : >"$LOG_OFF"
rm -rf "$DUMP_DIR"; mkdir -p "$DUMP_DIR"   # fresh PNG dump for the FuryPipe (compress) arm; the passthrough arm renders nothing
FURYPIPE_LOG="$LOG_ON"  FURYPIPE_PORT="$PORT_ON"  FURYPIPE_MODELS="$MODELS" FURYPIPE_DUMP_DIR="$DUMP_DIR" nohup node dist/node.js >/tmp/ec-on.log  2>&1 & disown
FURYPIPE_LOG="$LOG_OFF" FURYPIPE_PORT="$PORT_OFF" FURYPIPE_MODELS="$MODELS" FURYPIPE_DISABLE=1            nohup node dist/node.js >/tmp/ec-off.log 2>&1 & disown
sleep 2

echo "[5/5] seed two read-only working copies (context/ only)"
rm -rf /tmp/furypipe-ec-left /tmp/furypipe-ec-right
mkdir -p /tmp/furypipe-ec-left /tmp/furypipe-ec-right
cp -R "$EC/context" /tmp/furypipe-ec-left/context
cp -R "$EC/context" /tmp/furypipe-ec-right/context

# Record what we actually armed. a.sh/b.sh default to this model and refuse to run
# one the proxy would pass through uncompressed (which would look like a FuryPipe
# result while measuring nothing).
demo_write_state "$DEMO_NAME" "$DEMO_MODEL_BASE" "$DEMO_MODEL_ID" "$MODELS" "$ANSWER"

cat <<EOF

Ready. Proxies up: FuryPipe :$PORT_ON  ·  passthrough :$PORT_OFF
...
Rendered PNGs (what the FuryPipe model actually sees): $DUMP_DIR   (wiped + refilled each setup; passthrough arm renders none)

In a browser, open the live dashboard (context/token reduction, updates as it reads):
  http://localhost:$PORT_ON     # FuryPipe   -> "THIS SESSION — N% fewer tokens"
  http://localhost:$PORT_OFF    # plain    -> ~0% (the passthrough control)

Then, in TWO separate terminals:
  bash $EC/a.sh        # LEFT  = normal  (may DROWN in filler -> wrong integer)
  bash $EC/b.sh        # RIGHT = FuryPipe  (images filler, keeps needle as text -> ${ANSWER:-right})
