#!/usr/bin/env bash
# RIGHT column = FuryPipe (through the compress proxy, default :47824). Reads the SAME huge context.
# FuryPipe images the bulky filler but keeps the small needle as text, so it carries
# a smaller active context and should read the needle perfectly. Run
# `bash demo/effective-context/setup.sh` first.
set -uo pipefail

DIR=/tmp/furypipe-ec-right
...
# cover the model: FuryPipe would pass it through uncompressed and the run would look
# like a FuryPipe result while measuring nothing.
...
echo "RIGHT = FuryPipe (:$DEMO_PORT_ON), model=$DEMO_MODEL_ID. Launching interactive Claude with the needle task..."
