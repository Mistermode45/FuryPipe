...
**What it measures:** does FuryPipe stay *sharp* in a context big enough to overload
...
zone"). FuryPipe images the bulky filler (compressing it) but keeps the small
...
# Terminal 3 — RIGHT = FuryPipe   (interactive Claude — should answer correctly)
...
`claude-…` id; `FURYPIPE_DEMO_MODEL=opus bash demo/effective-context/setup.sh` works
...
isn't in scope: FuryPipe would pass it through uncompressed, so the run would look
like a FuryPipe result while measuring nothing. You can also toggle scope live on the
...
| column | expectation |
|---|---|
| **RIGHT — FuryPipe** (`b.sh`) | matches the ground truth (needle stays text) |
| LEFT — plain (`a.sh`) | **may be wrong** at this size (drowns in filler) |

**FuryPipe correct + plain wrong = the effective-context win.** Each proxy also serves
a live dashboard ([:48724](http://127.0.0.1:48724/) FuryPipe, [:48723](http://127.0.0.1:48723/)
...
- **The premise is UNVALIDATED.** "Plain drowns at this size and answers wrong" is an
  ...
  get it right. If it does, this demo shows **no FuryPipe advantage**. Run it and report
  *both* integers; don't assume the outcome.
- **The needle stays text on purpose.** FuryPipe is lossy on imaged content (it misreads
  exact strings), so this demo only works because the needle is small enough to stay
  text. It does **not** show FuryPipe recalling imaged exact values.
...
9975 / 45181`) preserved — FuryPipe through imaged spec + source. So *"compression
...
## The other demo
This is the **capability** demo. The **cost** demo ("does FuryPipe cost less on a real
