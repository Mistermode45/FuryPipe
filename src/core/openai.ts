...
import { buildPrecisionManifest } from './exact-guard.js';
import { createCompressionReceipt } from './receipt.js';
...
async function transformOpenAIResponsesCore(
  body: Uint8Array,
  opts: TransformOptions = {},
): Promise<{ body: Uint8Array; info: TransformInfo }> {
  const o = resolveOptions(opts);
...
type OpenAITransformResult = { body: Uint8Array; info: TransformInfo };

function attachOpenAIReceipt(
  result: OpenAITransformResult,
  original: Uint8Array,
  opts: TransformOptions,
  protocol: 'chat-completions' | 'responses',
): OpenAITransformResult {
  if (!opts.emitReceipt) return result;

  let precisionManifest: ReturnType<typeof buildPrecisionManifest> | undefined;
  try {
    precisionManifest = buildPrecisionManifest(new TextDecoder('utf-8', { fatal: true }).decode(original));
  } catch {
    // Keep byte-level verification available without claiming text spans for
    // malformed UTF-8 input.
    precisionManifest = undefined;
  }

  let model: string | undefined;
  try {
    const parsed = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(original)) as {
      model?: unknown;
    };
  ...
  result.info.receipt = createCompressionReceipt({
    original,
    ...
    cacheEffects: {
      protocol,
      imageCount: result.info.imageCount,
...
/** OpenAI Chat Completions wrapper with an opt-in plaintext-free receipt. */
export async function transformOpenAIChatCompletions(
  body: Uint8Array,
  opts: TransformOptions = {},
): Promise<OpenAITransformResult> {
  return attachOpenAIReceipt(
    await transformOpenAIChatCompletionsCore(body, opts),
    body,
    opts,
    'chat-completions',
  );
...
/** OpenAI Responses wrapper with an opt-in plaintext-free receipt. */
export async function transformOpenAIResponses(
  body: Uint8Array,
  opts: TransformOptions = {},
): Promise<OpenAITransformResult> {
  return attachOpenAIReceipt(
    await transformOpenAIResponsesCore(body, opts),
    body,
    opts,
    'responses',
  );
