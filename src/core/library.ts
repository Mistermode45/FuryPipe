...
import { createCompressionReceipt, type CompressionReceipt } from './receipt.js';
...
export interface PxpipeOptions
  extends Pick<
    TransformOptions,
    'charsPerToken' | 'historyAmortizationHorizon' | 'keepSharp' | 'emitRecoverable' | 'emitReceipt' | 'exactGuard'
  > {
...
export interface PxpipeTransformResult {
  ...
  /** Optional plaintext-free audit receipt; enabled only with `options.emitReceipt`. */
...
function buildReceipt(
  ...
  // Receipt generation is explicitly opt-in because exact-span scanning is
  // additional work on the request path. The manifest contains hashes and
  // offsets, never the source text itself.
  ...
  try {
    ...
    // A receipt can still prove byte hashes for non-UTF-8 input, but no
    // UTF-16 source span manifest is claimed in that case.
    precisionManifest = undefined;
  }
  return createCompressionReceipt({
    original,
    transformed,
