...
export default {
  async fetch(req: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    const configuredModels = (env.FURYPIPE_MODELS ?? env.PXPIPE_MODELS)?.trim();
    ...
    const renderCacheBytes = nonNegativeInt(env.FURYPIPE_RENDER_CACHE_BYTES ?? env.PXPIPE_RENDER_CACHE_BYTES);
    ...
    if (env.ANTHROPIC_API_KEY || env.OPENAI_API_KEY || env.CLOUDFLARE_API_TOKEN) {
      const workerSecret = env.FURYPIPE_WORKER_SECRET ?? env.PXPIPE_WORKER_SECRET;
      if (!workerSecret) {
        return new Response(
          JSON.stringify({
            error:
      ...
      const presented = req.headers.get('x-furypipe-secret') ?? req.headers.get('x-pxpipe-secret') ?? '';
    ...
    const trackingOn = truthy(env.FURYPIPE_TRACK ?? env.PXPIPE_TRACK, true);
    ...
    const sharedUpstream = env.FURYPIPE_UPSTREAM ?? env.PXPIPE_UPSTREAM;
    ...
    const config: ProxyConfig = {
      ...
      ...(positiveInt(env.FURYPIPE_MAX_REQUEST_BYTES ?? env.PXPIPE_MAX_REQUEST_BYTES) !== undefined
        ? { maxRequestBytes: positiveInt(env.FURYPIPE_MAX_REQUEST_BYTES ?? env.PXPIPE_MAX_REQUEST_BYTES) }
        : {}),
      transform,
      onRequest: (e) => {
