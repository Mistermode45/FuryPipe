...
| Integration | ordered merges in a dedicated worktree; conflicts stop; the base branch is never moved; `git worktree add/remove` are serialized per repository (process-wide) because git races on concurrent adds | `tests/fury-integrator.test.ts`, `tests/fury-run.test.ts`, `tests/fury-writer-pool.test.ts` |
...
| MCP configs | env/header values reduced to names; URL credentials and query strings stripped; secret-looking args redacted; probes send no configured secret, contact remote hosts only with `allowRemote`, and never start a project-scoped (repository-provided) stdio command until the operator marks it trusted; the confirmation shows the exact command; READ_ONLY allows only tools a trusted probe saw as read-only | `tests/fury-mcp-hub.test.ts` |
...
| Web fetch (SSRF) | public HTTP(S) on 80/443 only; private/loopback/link-local/metadata refused after DNS, including bracketed IPv6 literals and IPv6 forms embedding IPv4 (mapped, compatible, NAT64, 6to4); socket pinned to the validated address; every redirect revalidated; size/type caps; no cookies; no browser | `tests/fury-web.test.ts` |
