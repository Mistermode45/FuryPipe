...
The agent layer receives only a task digest in callback context and stores only metadata plus a host-owned opaque `contentHandle`. Lessons must pass the explicit validation callback before storage. The in-memory store is a test/runtime adapter, not durable production knowledge storage.
