...
This document describes FuryPipe's security boundaries and the assumptions that
...
- Anyone who can read the FuryPipe log directory or Workers Logs may learn
...
- Callers that know `FURYPIPE_WORKER_SECRET` are authorized to spend credentials
...
| --- | --- | --- |
| Public deployment spends a configured API key | Worker requires `FURYPIPE_WORKER_SECRET` and fails closed | Preserve the fail-closed check and constant-work comparison tests |
...
- FuryPipe does not authenticate the Node server or dashboard. Non-loopback
  deployment without an authenticated reverse proxy is unsafe by design.
- FuryPipe cannot protect content after it reaches a configured provider,
