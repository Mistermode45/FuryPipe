...
| Exact values / hashes / secrets | `PARTIAL_UPSTREAM` | Upstream possède factsheet/abstention et docs signalant une limite de rappel exact | `ExactGuard` local détecte/manifeste ; receipts opt-in des wrappers Anthropic/OpenAI ; branchement de protection automatique dans tout `transformRequest` encore ouvert |
| Recovery byte-exact | `LOCAL_SLICE` | `src/core/recovery-store.ts` ; CAS/namespace/intégrité et receipts opt-in testés | Pas BLAKE3, zstd, SQLite, backup drill ni intégration des handles dans les transforms |
