...
| Recovery byte-exact | `PARTIAL` | `src/core/recovery-store.ts` ; CAS/namespace/intégrité, quotas global/namespace, orphan/TTL GC, backup/restore vérifiés, AES-256-GCM optionnel, key-ring/rekey, legacy fail-closed et reprise de manifest simulée | Pas BLAKE3, zstd ou SQLite ; ACL Windows, kill process réel, multi-process locking, corruption filesystem et émission automatique par transform restent ouverts |
