...
| Recovery byte-exact | `PARTIAL` | `src/core/recovery-store.ts` ; CAS/namespace/intégrité, quotas global/namespace, orphan/TTL GC, backup/restore vérifiés, AES-256-GCM optionnel, key-ring/rekey, reprise de manifest simulée, verrou racine inter-processus, listing borné par metadata | Pas BLAKE3, zstd ou SQLite ; ACL Windows, kill process réel pendant chaque phase, corruption filesystem systématique et émission automatique par transform restent ouverts |
