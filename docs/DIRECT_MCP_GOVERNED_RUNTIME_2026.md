...
7. unforgeable short-lived execution permit;
8. synchronous one-time permit consumption before a future transport call;
9. execution state;
10. separate verification state.
...
This module imports no MCP client SDK and cannot call a tool.
...
A call is allowed only after an input-bound permit is created and synchronously
consumed/revalidated immediately before exactly one `callTool()`. Returning a
result sets `executed`; protocol/result semantics determine `succeeded`;
neither implies `verified`.
...
- lifecycle state copies are not authoritative process-local evidence;
- permits cannot be forged, copied, replayed or rebound across source, endpoint,
  tool schema, input, policy decision or approval kind;
- permits expire after a bounded lifetime (30 seconds by default, 60 seconds max);
