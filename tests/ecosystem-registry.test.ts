...
import { evaluateFuryTrust, isGeneratedFuryTrustReport } from '../src/fury-trust.js';
...
describe('ecosystem deduplication and registry', () => {
  ...
  it('rejects copied or caller-forged trust reports from registry storage', () => {
    ...
    const stored = registry.getTrustReport(report.candidateId);
    expect(stored).toBe(report);
    expect(isGeneratedFuryTrustReport(stored)).toBe(true);
  });
});
