...
describe('warp route port selection', () => {
  ...
  it('honours the port when the pattern names one', () => {
    ...
    // Same host, different port: the FuryPipe front door must not divert to itself.
    expect(matchRoute(routes, '127.0.0.1:48721', '/v1/responses')).toBeNull();
    expect(hostCouldMatch(routes, '127.0.0.1:48721')).toBe(false);
  });
  ...
  it('preserves the path and query when rewriting', () => {
    ...
    expect(rewriteUrl(route, '/v1/responses?stream=true')).toBe(
      'http://127.0.0.1:48721/v1/responses?stream=true',
    );
  });
