...
describe('proxy usage extraction', () => {
  ...
  it('preserves Codex ChatGPT OAuth when OPENAI_API_KEY is configured', async () => {
    ...
    const res = await proxy(new Request('http://localhost/backend-api/codex/responses', {
      method: 'POST',
      headers: {
        // ChatGPT OAuth access tokens are JWTs; the shape-based credential
        // policy (resolveOpenAIRouteAuth) only keeps JWT-shaped bearers.
        authorization: 'Bearer eyJhbGciOiJSUzI1NiJ9.eyJzdWIiOiJjb2RleCJ9.sig',
    ...
    expect(upstreamAuth).toBe('Bearer eyJhbGciOiJSUzI1NiJ9.eyJzdWIiOiJjb2RleCJ9.sig');
  });
  ...
  it('rejects an oversized chunked transform request before calling upstream', async () => {
    ...
    expect(payload.error?.message).toContain('16777216-byte');
