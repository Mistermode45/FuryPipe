 ...
 * rather than extend it. A FuryLink CA file holding only our root made every
 ...
 *
 * Run just this file:  pnpm vitest run tests/fury-link-ca-bundle.test.ts
 */
import { afterEach, describe, expect, it } from 'vitest';
import { mkdtempSync, readFileSync, rmSync, writeFileSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { CertificateAuthority, findSystemRootBundle } from '../src/fury-link/ca.js';

const CERT_RE = /-----BEGIN CERTIFICATE-----/g;
const count = (pem: string): number => (pem.match(CERT_RE) ?? []).length;

describe('FuryLink CA bundle (#245)', () => {
  const dirs: string[] = [];
  ...
  const tmp = (): string => {
    const d = mkdtempSync(join(tmpdir(), 'FuryPipe-furylink-ca-'));
    dirs.push(d);
    return d;
  };

  it('keeps furylink-ca.pem CA-only and writes a separate bundle', () => {
  ...
  it('does not nest its own bundle when SSL_CERT_FILE already points at it (FuryLink restart)', () => {
    const d = tmp();
    delete process.env.SSL_CERT_FILE;
