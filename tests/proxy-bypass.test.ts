...
// Deliberately OUT of the pinned FURYPIPE_MODELS scope: without bypass the
...
describe('x-furypipe-bypass opt-out', () => {
  it('skips transform classification and forwards the body byte-for-byte', async () => {
    const { upstreamRequests, captured, status } = await roundTrip({ 'x-furypipe-bypass': '1' });

    expect(status).toBe(200);
    expect(upstreamRequests).toHaveLength(1);
    // Body reaches the upstream untouched.
    expect(await upstreamRequests[0].text()).toBe(OUT_OF_SCOPE_BODY);
    // The FuryPipe-only signal is never forwarded upstream.
    expect(upstreamRequests[0].headers.get('x-furypipe-bypass')).toBeNull();
  ...
  it('treats any non-falsy value as bypass', async () => {
    const { upstreamRequests, captured } = await roundTrip({ 'x-furypipe-bypass': 'true' });
    expect(await upstreamRequests[0].text()).toBe(OUT_OF_SCOPE_BODY);
    expect(captured?.info?.reason).toBeUndefined();
  });

  it.each(['0', 'false', 'off', 'no'])('does not bypass for falsy value %j', async (v) => {
    const { upstreamRequests, captured } = await roundTrip({ 'x-furypipe-bypass': v });
