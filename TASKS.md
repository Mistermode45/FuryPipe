...
| M8 MCP current + legacy | JSON-RPC stdio legacy local, revue spec moderne effectuée | stdio local | handshake/list/call/binary/range/delete | CI package smoke | tailles bornées, metadata scalaires | COMPATIBILITY, docs/MCP.md | PARTIAL | `tests/mcp.test.ts` ; spec 2026-07-28 | serveur moderne SDK, HTTP sécurisé/OAuth et conformance restent à implémenter |
...
| M10 CI cross-platform | workflows présents, matrice Node 22/24/26, package smoke réel | Windows local + Ubuntu/Windows/macOS distants | frozen install/typecheck/tests/build/pack/install/CLI/doctor/MCP | 9/9 jobs verts sur push et PR | audit local | TESTING | TESTED_REMOTE | `.github/workflows/ci.yml`, `scripts/package-smoke.mjs`, runs 34628201836/34628205790 | relancer après le prochain push |
