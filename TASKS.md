...
| M4 Recovery core | CAS SHA-256, namespace, atomicité, quotas, TTL/GC, backup/restore sans écrasement | filesystem local | bytes, integrity, isolation, quota, GC, backup/restore | non exécutée | tenant namespace et limites | ARCHITECTURE, SECURITY | TESTED_LOCAL | `tests/recovery-store.test.ts` | chiffrement/ACL/SQLite/orphan GC à faire |
